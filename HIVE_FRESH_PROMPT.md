...
- **HiveRelay Endpoint**: `https://relay.hivetalk.org` (production). Override with `VITE_HIVERELAY_API_BASE` / `VITE_HIVERELAY_RELAY_URL` (e.g. staging: `https://l402.exe.xyz` / `wss://l402.exe.xyz/`). The relay URL is **not** hardcoded — it is read from env vars with the production relay as the default. See `.env.example` for all available env vars.
